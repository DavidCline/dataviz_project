...
This is the data visualization project for the data visualization course in Jeff Leek's CBDS course series.
...
# had to put in the complete path in order to make the read statement work
...
## Let's examine the 2016 spike in licenses for dogs and cats separately
pets %>% 
  group_by(species, ym) %>% 
  filter(grepl("2016", ym)) %>% 
  count(n=n()) %>% 
...
ggsave(plot = p, filename = "/cloud/project/dataviz_project/figures/explanatory_figures/steak_R.png", width = 8, height = 4)
```
...
### Explore the data -- create your own plot!
## Re-wrangle the steak data set
pref_by_gender <- steak %>% 
  mutate(steak_pref = factor(How.do.you.like.your.steak.prepared.,
                             levels = c("Well",
                                        "Medium Well",
                                        "Medium",
                                        "Medium rare",
                                        "Rare"))) %>% 
  filter(steak_pref != "" & Gender != "") %>% 
  group_by(Gender, steak_pref) %>% 
  summarise(n = n()) %>%
  mutate(prop = n / sum(n))
## generate the plot 
p_DRC <- ggplot(pref_by_gender) +
  ## specify you want to generate a bar chart
  geom_bar(aes(x = steak_pref, y = prop, fill = steak_pref),
                stat = 'identity', 
                width = 0.7) + 
  ...
  ## change the title, subtitle, and caption
  labs(title="'How Do You Like Your Steak Prepared?'", 
                subtitle="From a survey of 432 steak-eating Americans, by gender",
                caption="FiveThirtyEight: Surveymonkey") +
  ## change the theme (use ggthemes)
  ggthemes::theme_fivethirtyeight() +
  ## fine tune the theme
...
## save plot to figures/explanatory_figures directory
ggsave(plot = p_DRC, filename = "/cloud/project/dataviz_project/figures/explanatory_figures/steak_DRC.png")
```
